/*
ServerPerks webadmin interface
*/
class ServerPerkWebAdmin extends xWebQueryHandler;
...
function bool Init()
{
	foreach Level.AllActors( class'ServerPerksMut', Mut )
		break;
	return true;
}
function Cleanup()
...
function RequestMain(WebRequest Request, WebResponse Response)
{
	...
	SendHeaderLine(Response,"ServerPerks settings");
...
function RequestPerks(WebRequest Request, WebResponse Response)
{
	...
	if( Request.GetVariable("submit") == "Submit" )
	{
		...
		if( S!="" )
		{
			...
			Class'ServerPerksMut'.Default.Perks.Length = 0;
			for( i=0; i<PA.Length; ++i )
			{
				VC = class<SRVeterancyTypes>(DynamicLoadObject(PA[i],Class'Class'));
				if( VC!=None )
				{
					Class'ServerPerksMut'.Default.Perks.Length = j+1;
					Class'ServerPerksMut'.Default.Perks[j++] = string(VC);
				}
				else if( Er=="" )
					Er = PA[i];
				else Er $= ", "$PA[i];
			}
			Class'ServerPerksMut'.Static.StaticSaveConfig();
		}
	}
	...
	// Send perk list
	for( i=0; i<Class'ServerPerksMut'.Default.Perks.Length; ++i )
	{
		if( i==0 )
			S = Class'ServerPerksMut'.Default.Perks[0];
		else S = S$Chr(10)$Class'ServerPerksMut'.Default.Perks[i];
	}
...
function TradetCat(WebRequest Request, WebResponse Response)
{
	...
	if( Request.GetVariable("submit") == "Submit" )
	{
		for( i=Class'ServerPerksMut'.Default.WeaponCategories.Length; i>=0; --i )
		{
			...
			if( S=="" || S=="NEW" )
			{
				if( i<Class'ServerPerksMut'.Default.WeaponCategories.Length )
					Class'ServerPerksMut'.Default.WeaponCategories.Remove(i,1);
			}
			else
			{
				if( j>=0 )
					S = string(j)$":"$S;
				if( i>=Class'ServerPerksMut'.Default.WeaponCategories.Length )
					Class'ServerPerksMut'.Default.WeaponCategories.Length = i+1;
				Class'ServerPerksMut'.Default.WeaponCategories[i] = S;
			}
		}
		Class'ServerPerksMut'.Static.StaticSaveConfig();
	}
	...
	for( i=0; i<Class'ServerPerksMut'.Default.Perks.Length; ++i )
		D = D$"<option value=\""$i$"\">"$Class'ServerPerksMut'.Default.Perks[i]$"</option>";
	D $= "<option value=\""$i$"\">Unperked</option></select>";

	// categories
	Response.SendText("<tr>"$S$"</tr>");
	for( i=0; i<=Class'ServerPerksMut'.Default.WeaponCategories.Length; ++i )
	{
		...
		{
			if( Divide(Class'ServerPerksMut'.Default.WeaponCategories[i],":",A,B) )
			{
				j = int(A);
				if( j<Class'ServerPerksMut'.Default.Perks.Length )
					A = ">"$Class'ServerPerksMut'.Default.Perks[j]$"<";
				else A = ">Unperked<";
				S = Repl(S,A," selected=\"selected\""$A); // Shitty hack.
			}
			else B = Class'ServerPerksMut'.Default.WeaponCategories[i];
		}
...
function RequestInventory(WebRequest Request, WebResponse Response)
{
	...
	if( Request.GetVariable("submit") == "Submit" )
	{
		...
		if( S!="" )
		{
			...
			Class'ServerPerksMut'.Default.TraderInventory.Length = 0;
			for( i=0; i<PA.Length; ++i )
			{
				if( !Divide(PA[i],":",A,B) )
				{
				...
				if( PC!=None )
				{
					Class'ServerPerksMut'.Default.TraderInventory.Length = j+1;
					Class'ServerPerksMut'.Default.TraderInventory[j++] = A$string(PC);
				}
				else if( Er=="" )
					Er = PA[i];
				else Er $= ", "$PA[i];
	...
	// Send perk list
	for( i=0; i<Class'ServerPerksMut'.Default.TraderInventory.Length; ++i )
	{
		if( i==0 )
			S = Class'ServerPerksMut'.Default.TraderInventory[0];
		else S = S$Chr(10)$Class'ServerPerksMut'.Default.TraderInventory[i];
	}
...
defaultproperties
{
     HomeURL="ServerPerkMain"
     PerksURL="ServerPerksPerks"
     TraderCatURL="ServerPerksTraderCat"
     TraderInvURL="ServerPerksTraderInv"
     DefaultPage="ServerPerkWebAdmin"
     Title="Server Perks"
}
