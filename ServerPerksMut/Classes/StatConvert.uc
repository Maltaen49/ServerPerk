class StatConvert extends Commandlet;
...
function int Main( string Parms )
{
	...
	local int i,j,z;
	local StatsObject A,B;
	local string NewID;

	List = GetPerObjectNames("ServerPerksStat","StatsObject",9999999);
	for( i=0; i<List.Length; ++i )
	{
		...
		A = new(None,List[i]) Class'StatsObject';
		B = new(None,NewID) Class'StatsObject';
...
defaultproperties
{
     HelpCmd="Convert ServerPerks stats LinuxID to WindowsID."
     HelpUsage="Run UCC.exe ServerPerksMut.StatConvert /norunaway"
