class StimulatorFire extends StimulatorAltFire;
...
function AttemptHeal()
{
    ...
    {
        super.ModeDoFire();
        Stimulator(Weapon).ClientSuccessfulHeal(CachedHealee.PlayerReplicationInfo.PlayerName);
    }
    else
    {
...
Function Timer()
{
	...
	HealSum = Stimulator(Weapon).HealBoostAmount;
	...
	SRHumanPawn(Healed).Stimulate( HealSum, 30.0 + 1.0 * KFPlayerReplicationInfo(Instigator.PlayerReplicationInfo).ClientVeteranSkillLevel );
...
event ModeDoFire()
{
	...
	{
       Stimulator(Weapon).ServerAttemptHeal();
	}
}
...
defaultproperties
{
     ...
     FireRate=2.800000
     AmmoPerFire=1
	 Ammoclass=class'StimulatorAmmo'
}
