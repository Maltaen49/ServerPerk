Class UChimera extends KFMonster;

var byte SightFXCounter;
var UseTriggerCatcher UseCatcher;
...
simulated function PostBeginPlay()
{
	...
	{
    		UseCatcher = Spawn(class'UseTriggerCatcher',self,,Location + (vect(-50,0,0) >> Rotation));
        	UseCatcher.SetBase( self );
	}
	if( Level.NetMode!=NM_DedicatedServer )
		Spawn(Class'ChimeraDieFX');
	Super.PostBeginPlay();
}
...
function BiteTarget()
{
	...
	{
		NC = C.nextController;
		if( C.Pawn!=None && UChimera(C.Pawn)==None && C.Pawn.Health>0 && BiteHits(C.Pawn) )
		{
...
function Died(Controller Killer, class<DamageType> damageType, vector HitLocation)
{
	...
	if( bMessageDeath && Killer!=None && Killer!=Controller && Killer.PlayerReplicationInfo!=None )
		BroadcastLocalizedMessage(Class'ChimeraSummonMessage',2,Killer.PlayerReplicationInfo);
...
{
	if( Level.NetMode!=NM_DedicatedServer )
		Spawn(Class'ChimeraNotice',Self);
	if( Level.NetMode!=NM_Client )
	{
		Acceleration = vect(0,0,0);
...
{
     MeleeDamage=9999
     CurrentDamType=Class'DamTypeChimeraBit'
     ...
     ControllerClass=Class'ChimeraController'
     bDoTorsoTwist=False
     MovementAnims(0)="Run"
     MovementAnims(1)="Run"
     MovementAnims(2)="Run"
     ...
     Mesh=SkeletalMesh'MutSlotMachineSR.UChimMesh'
     PrePivot=(Z=-9.000000)
     Skins(0)=Shader'MutSlotMachineSR.Skins.UCEyeShader'
     bNetNotify=False
