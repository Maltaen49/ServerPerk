Class ChimeraController extends KFMonsterController;
...
{
	if( NewEnemy==None || NewEnemy.Health<=0 || UChimera(NewEnemy)!=None || !LineOfSightTo(NewEnemy) )
		return false;
	if( Enemy!=None && NewEnemy!=None && NewEnemy!=Enemy && NewEnemy.Controller!=None )
	{
		if( LineOfSightTo(Enemy) && VSize(Enemy.Location-Pawn.Location)<VSize(NewEnemy.Location-Pawn.Location) )
			Return False;
		Enemy = None;
	}
	if( Super.SetEnemy(NewEnemy,true) )
	{
...
Begin:
	...
	{
		Pawn.Died(None,Class'DamTypeShutDown',Pawn.Location);
		Stop;
	}
	WaitForLanding();
...
Begin:
	RoamedEnemy = Enemy;
	UChimera(Pawn).HandleSighted();
	Focus = Enemy;
	FinishRotation();
	UChimera(Pawn).PlayRoamAnim();
	while( KFM.bShotAnim )
		Sleep(0.1f);
	FightEnemy(true);
}
