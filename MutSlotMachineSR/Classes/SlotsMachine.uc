...
var config float IconScaling;

var array< class<SlotCard> > CardsList;

var bool bDrawCards,bBotOwner;
...
{
	bBotOwner = (PlayerController(Owner)==None);
	if( bBotOwner )
...
{
	Disable('Tick');
	if( Level.NetMode!=NM_DedicatedServer && !bBotOwner )
		SetTimer(0.1,true);
}
...
function DrawNextCard()
{
	...
	// else Level.Game.Broadcast(Self,Controller(Owner).PlayerReplicationInfo.PlayerName@"got"@Cards[CardNum].Name);
...
final function class<SlotCard> PickRandomCard()
{
	...
	{
		if( bBotOwner && CardsList[i].Default.bNotForBots )
			continue;
		if( i==LastUsedIndex )
...
Begin:
	...
	if( Cards[0]==Cards[1] && Cards[1]==Cards[2] )
	{
		...
		while( Owner==None || Controller(Owner).Pawn==None )
			Sleep(1.f);
