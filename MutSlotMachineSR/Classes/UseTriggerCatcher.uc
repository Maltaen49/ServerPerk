class UseTriggerCatcher extends ExtendedZCollision;
...
function UsedBy( Pawn user )
{
	...
	Pawn(Owner).Died(user.Controller,Class'DamTypeShutDown',user.Location);
}
...
function Touch( Actor Other )
{
	...
	if( PlayerController(Pawn(Other).Controller)!=none && LastMessageTimer<Level.TimeSeconds )
	{
		...
		PlayerController(Pawn(Other).Controller).ReceiveLocalizedMessage(class'ChimeraMessage', 6);
	}
}
